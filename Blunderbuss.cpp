...
struct TranspositionTableEntry {
    ...
    //TranspositionTableEntry(int dpth, int vl, NodeType tp, Move move)
    //    : depth(dpth), value(vl), type(tp), best_move(move) {}
...
struct Board 
{
    ...
    void get_knight_moves(std::vector<Move>& or_moves, int square, bool color) //0 = white, 1 = black
    {
        uint64_t result;
        uint64_t captures;
...
{
    Board board;
    board.setup_board_from_fen("rnbqkbnr/pppp1p1p/8/4pP2/8/8/PPPPP1PP/RNBQKBNR w KQq e6 0 2");
    ...
    while (true) 
    {
        //std::vector<Move> moves1 = board.get_legal_moves(1);
        //board.make_move(moves[14]);
        //board.unmake_move();

        //board.if_check(0);
        //board.if_check(1);
        uint64_t hash = board.compute_zobrist_hash();
        ++count;
    ...
    std::cout << "The function was executed " << count << " times in one second." << std::endl;
    return;
}
